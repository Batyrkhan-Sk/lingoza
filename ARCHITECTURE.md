...
| Displayable lyrics | Implement `LicensedLyricsProvider` (`packages/content/src/sources/lyrics.ts`) against the aggregator you have licensed and pass it as `MediaOptions.licensedLyricsProviders`. It is tried before the analysis-only pair and marks its result `displayable`, which is the only thing that lets an interface print a line — no renderer changes. |
