...
| Displayable lyrics | Two routes, both off by default. If you hold display rights yourself, set `LYRICS_DISPLAY_ATTRIBUTION` to the credit your licence requires (and `LYRICS_DISPLAY_MAX_LINES` for an excerpt licence). If your rights come through an aggregator's API, implement `LicensedLyricsProvider` (`packages/content/src/sources/lyrics.ts`) and pass it as `MediaOptions.licensedLyricsProviders`; it is tried first and carries its own attribution. Either route sets `displayable`, which is the only thing any renderer consults. |
...
- **Lyrics are analysed, never reproduced.** LRCLIB and lyrics.ovh serve publisher-owned text without a distribution licence, so `studySong` reads a song's lines, derives coverage, vocabulary, grammar and pace from them, and drops them inside one function call. Nothing downstream — cache, database, route or chat message — holds a lyric line, and `LyricsAnalysisInput` is not exported from `@lingoza/content`'s index for that reason. Reading a song *as text* therefore runs on lines the learner supplies (`/lyrics`, or simply pasting them), which needs no licence in the same way a dictionary needs none. The gate is a single flag, `displayable`, and nothing sets it unless the operator has declared rights — a `LicensedLyricsProvider` that carries its own, or `LYRICS_DISPLAY_ATTRIBUTION` naming the credit theirs requires. When either is present the same line-by-line reader runs on fetched lyrics with nothing else changed; the excerpt cap bounds display only, since analysis is entitled to the whole work.
